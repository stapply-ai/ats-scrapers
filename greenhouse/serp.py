...
def read_existing_urls(csv_file):
    ...
    if os.path.exists(csv_file):
        try:
            ...
            if "greenhouse_url" in df.columns:
                existing_urls = set(df["greenhouse_url"].dropna().tolist())
...
def fetch_urls():
    ...
    for i in range(PAGES):
        ...
        for res in results.get("organic_results", []):
            link = res.get("link")
            print(link)
            if link and "job-boards.greenhouse.io" in link:
                m = re.match(r"(https://job-boards\.greenhouse\.io/[^/?#]+)", link)
                if m:
...
def save_to_csv(urls, existing_urls, output_file):
    ...
    df = pd.DataFrame({"greenhouse_url": sorted(all_urls)})
    df.to_csv(output_file, index=False)
    print(f"✅ Saved {len(df)} companies to {output_file}")


if __name__ == "__main__":
    output_file = "greenhouse_companies_serpapi.csv"
