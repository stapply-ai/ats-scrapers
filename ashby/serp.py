...
def read_existing_urls(csv_file):
    ...
    if os.path.exists(csv_file):
        try:
            ...
            if "ashby_url" in df.columns:
                existing_urls = set(df["ashby_url"].dropna().tolist())
...
def fetch_urls():
    ...
    for i in range(PAGES):
        ...
        for res in results.get("organic_results", []):
            link = res.get("link")
            print(link)
            if link and "jobs.ashbyhq.com" in link:
                m = re.match(r"(https://jobs\.ashbyhq\.com/[^/?#]+)", link)
                if m:
...
def save_to_csv(urls, existing_urls, output_file):
    ...
    df = pd.DataFrame({"ashby_url": sorted(all_urls)})
    df.to_csv(output_file, index=False)
    print(f"✅ Saved {len(df)} companies to {output_file}")


if __name__ == "__main__":
    output_file = "ashby_companies_serpapi.csv"
